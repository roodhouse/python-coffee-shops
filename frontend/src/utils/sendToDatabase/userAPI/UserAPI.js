export const updateUser = async (user_id, venue, placeId, type) => { 
    try {
        ...
        if ( getCurrentUserReviews.ok ) {
            ...
            if (type === 'new') {
                updatedReviewIds = [ ...currentReviews, placeId]
            } else if (type === 'full') {   
                updatedReviewIds = currentReviews.filter(item => item !== placeId)                
            }
